...
- [ ] T116 [US7] Implement LLM proxy + policy chain (`POST /llm/proxy`: authenticate PAT, enforce `allowed_tools`/token budget, deduct credits, resolve alias, forward **as a synchronous HTTP streaming pass-through to the Python gateway `:8000`** — `stream:true` relays provider SSE verbatim, flush-per-chunk, no buffering; client disconnect cancels `request.Context()` → forwarded HTTP request → `chat_stream`; not gRPC/NATS, research §20 — then trace; admin-disableable BYOK) in `backend-go/internal/policy/service/llm_proxy.go` (FR-026, FR-027)
